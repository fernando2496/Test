...
// FIX 1: reemplaza escape()/unescape() deprecados por TextDecoder/TextEncoder
...
function AppWrapper() {
  // ================================================================
  const GH_TOKEN = "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";
  const GH_OWNER = "fernando2496";
  const GH_REPO  = "Test";
  // ================================================================
  ...
  // FIX 2: refs para evitar stale closure en el debounce
  ...
  // FIX 3: flag para evitar que el intervalo de 30s dispare saves en loop al fallar
  const isSavingRef = React.useRef(false);

  const loadFromGitHub = React.useCallback(async () => {
    if (isSavingRef.current) return; // no recargar mientras se está guardando
  ...
  const saveToGitHub = React.useCallback(async (type, value) => {
    ...
    pendingRef.current = setTimeout(async () => {
      ...
      setSyncing(true); setSyncError(null);
      // FIX 2: leer de refs para tener siempre el estado más reciente
      const snap = {
  ...
  }, []); // dependencias vacías: lee de refs, no del closure
